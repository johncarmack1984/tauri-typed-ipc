...
//! generates from the `Faders` trait, using ttipc's own `check` drift
//! guard. Regenerate with `REGEN_BINDINGS=1 cargo test`; a plain `cargo
//! test` fails on drift, which is the CI guard against hand-edits and
//! stale bindings.
...
fn bindings_match_generator() {
    let bindings = faders_lib::ttipc_bindings();
